...
模型配置采用 OpenCode 风格的“外层 provider id + 内层 SDK package + models map”结构。外层 provider 可以叫 `mimo`、`mimo-claude`、`gateway-prod`，不强行叫 `openai` 或 `anthropic`。真正决定 SDK 路径的是 `npm`，请求形态由 `options.mode` 控制。
...
- `adapter`: 从 `npm` 解析得到的 `@ai-sdk/openai-compatible` / `@ai-sdk/openai` / `@ai-sdk/anthropic`。
- `mode`: `chat` / `responses` / `messages`。
- `baseUrl`, `apiKey`, `authToken`, `maxOutputTokens`。

模型 key 默认就是真实模型 ID，`name` 只用于 UI 展示，不再推荐 `fast -> model` 这种别名层。
